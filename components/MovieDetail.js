...
export default function MovieDetail() {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const tmdbLanguage = language === 'tr' ? 'tr-TR' : 
                            language === 'en' ? 'en-US' : 
                            language; // Eğer başka format gelirse olduğu gibi kullan
  ...
  }, [id, language]); // language dependency'sini ekledik

  if (!movie) {
    return (
      <View style={styles.center}>
        <Text>{t('movieDetail.loading')}</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>
              {t('movieDetail.release_date')}: {movie.release_date}  •  {movie.runtime} {t('movieDetail.minutes')}
            </Text>
        ...
        <Text style={styles.sectionTitle}>{t('movieDetail.summary')}</Text>
